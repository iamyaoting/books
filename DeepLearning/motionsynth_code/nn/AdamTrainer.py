...
class AdamTrainer:
    ...
    def train(self, network, input_data, output_data, filename=None, logname=None, restart=True, shuffle=True):
        ...
        for epoch in range(self.epochs):
            ...
            #sys.stdout.write('\n')
            #Used to add new objects at the end of the list
            
            c = []
            for bii, bi in enumerate(batchinds):
                #Used to add new objects at the end of the list
                #bii is the index of the element in the list
                #bi is the element
                ...
                #if True:
                    sys.stdout.write('[Epoch %i]  %0.1f%% mean %.5f\n' % (epoch, 100 * float(bii)/len(batchinds), np.mean(c)))
                    sys.stdout.flush()
 
            curr_mean = np.mean(c)
            diff_mean, last_mean = curr_mean-last_mean, curr_mean
            sys.stdout.write('[Epoch %i] 100.0%% mean %.5f diff %.5f %s\n' % 
